<template>
    ...
    <div class="padding-bottom-10">
      <el-alert
        ...
        type="info"
        show-icon
        description="此处可以登记药品投喂"
      >
      </el-alert>
      <el-table
        v-loading="loading"
        :data="costMedicineList"
        class="margin-top-10"
      >
        ...
        <el-table-column label="备注" align="center" prop="medicineCount" /> 
        <el-table-column
          label="操作"
          align="center"
        ...
        >
          <template #header>
            <el-tag
              @click="handleAdd"
              v-hasPermi="['agriculture:costMedicine:add']"
          ...
          <template slot-scope="scope">
            <el-button
              ...
              v-hasPermi="['agriculture:costMedicine:edit']"
              >修改</el-button
            >
            <el-button
              size="mini"
              ...
              v-hasPermi="['agriculture:costMedicine:remove']"
              >删除</el-button
            >
          </template>
        </el-table-column>
      ...
      <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="80px">
          <el-form-item label="药品名称" prop="medicineId">
            ...
            >
              <el-option
                v-for="item in medicineInfoList"
                :key="item.medicineId"
          ...
          <el-form-item label="使用数量" prop="medicineCount">
            <el-input v-model="form.medicineCount" placeholder="请输入使用数量" />
          </el-form-item>
          ...
          <el-form-item label="备注" prop="medicineCount">
            <el-input v-model="form.medicineCount" placeholder="请输入备注" />
          </el-form-item>
        </el-form>
  ...
  import {
    ...
    addCostMedicine,
    updateCostMedicine,
  } from "@/api/fishingGround/costMedicine";
  import { listMedicineInfo } from "@/api/fishingGround/MedicineInfo";
  ...
  export default {
    ...
    created() {
      this.getList();
      this.getMedicineInfoList();
    },
    methods: {
      /** 查询药品用量列表 */
      getList() {
        this.loading = true;
        listCostMedicine(this.queryParams).then((response) => {
          this.costMedicineList = response.rows;
      ...
      /** 查询药品列表 */
      getMedicineInfoList() {
        listMedicineInfo().then((response) => {
      ...
      handleUpdate(row) {
        ...
        getCostMedicine(costId).then((response) => {
        ...
        this.$refs["form"].validate((valid) => {
          if (valid) {
            if (this.form.costId != null) {
              updateCostMedicine(this.form).then((response) => {
              ...
              addCostMedicine(this.form).then((response) => {
      ...
      handleDelete(row) {
        ...
        this.$modal
          ...
          .then(function () {
            return delCostMedicine(costIds);
          })
          .then(() => {
            this.getList();
      ...
      handleExport() {
        this.download(
          "agriculture/costMedicine/export",
          {
            ...this.queryParams,
          },
          `costMedicine_${new Date().getTime()}.xlsx`
        );
      },
    },
  };
